...
class GameRouter
{
    ...
    private async SaveChildGames(game: IGame):Promise<any> {
        ...
        for(let i = 0; i < gamesNeeded; i++){
            ...
            let promise = this.Sheets.createTeamSheet(game.Location + game.DatePlayed ? game.DatePlayed.toLocaleString() : new Date().toLocaleString() + " Team " + (i + 1), game.SourceSheetId)
